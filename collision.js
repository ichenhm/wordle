...
var Mask={
	...
	intersect : function(x, y, oth){
		oth.left+=x;
		oth.top+=y;
		i_left=max(this.left, oth.left);
		i_right=min(this.left + this.width, oth.left + oth.width);
		
		i_top=max(this.top, oth.top);
		i_bottom=min(this.top + this.height, oth.top + oth.height);
		...
		for (var i = i_left; i < i_right; i++)
			for (var j = i_top; j < i_bottom; j++)
				if (this.pix[j-this.top][i-this.left] > 0 && oth.pix[j-oth.top][i-oth.left] > 0) return true;
		return false;
	}
};